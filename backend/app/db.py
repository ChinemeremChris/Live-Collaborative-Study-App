...
from sqlalchemy import String, Boolean, ForeignKey, Text, Float, Date, Integer
...
class Deck(Base):
    ...
    child_cards: Mapped[list["Card"]] = relationship(back_populates="parent_deck")
    deck_tag: Mapped[list["Deck_Tag"]] = relationship(back_populates="deck", cascade="all, delete-orphan")
    deck_session: Mapped[list["StudySession"]] = relationship(back_populates="deck")
    rooms_studied: Mapped[list["Room"]] = relationship(back_populates="deck")
    deck_rating: Mapped[list["DeckRating"]] = relationship(back_populates="deck")

class DeckRating(Base):
    __tablename__ = "deck_rating"
    deck_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("deck.deck_id"), primary_key=True, index=True)
...
class Card(Base):
    ...
    deck_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("deck.deck_id"), index=True)
    ...
    card_progress: Mapped[list["CardProgress"]] = relationship(back_populates="card")
...
class Deck_Tag(Base):
    ...
    tag_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("tag.tag_id"), primary_key=True)
    deck_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("deck.deck_id"), primary_key=True)
...
class CardProgress(Base):
    ...
    last_rating: Mapped[str] = mapped_column(String(6))
...
class Room(Base):
    ...
    deck_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("deck.deck_id"))
...
class RoomParticipant(Base):
    ...
    room_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("room.room_id"), primary_key=True, index=True)
    score: Mapped[int] = mapped_column(Integer)
    placement: Mapped[str] = mapped_column(String(10)) #save position like 1/30 as string so you don't need complicated queries
